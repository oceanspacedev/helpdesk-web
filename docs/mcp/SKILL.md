...
2. Forward only `content[0].text`; if unavailable, forward `structuredContent.user_reply`.
...
Forward phone and OTP questions exactly. A `phone`, `reporter_name`, or `channel=whatsapp` argument alone is not proof. Only a trusted WhatsApp webhook gateway may generate and inject the one-event `identity_assertion` together with `external_message_id`; the model must never generate or request it.

Do not answer form questions yourself, classify the issue, skip a step, choose list values, or invent an `HD-...` number. The Helpdesk server owns validation, confirmation, ticket creation, and retry handling.
